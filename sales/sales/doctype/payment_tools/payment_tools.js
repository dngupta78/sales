...
// Help content
frappe.ui.form.on("Payment Tools", "onload", function(frm) {
...
frappe.ui.form.on("Payment Tools", "refresh", function(frm) {
	frappe.ui.form.trigger("Payment Tools", "party_type");
});

frappe.ui.form.on("Payment Tools", "party_type", function(frm) {
	frm.toggle_reqd("customer", frm.doc.party_type == "Customer");
	frm.toggle_reqd("supplier", frm.doc.party_type == "Supplier");
});

frappe.ui.form.on("Payment Tools", "company", function(frm) {
	erpnext.payment_tool.check_mandatory_to_set_button(frm);
});

frappe.ui.form.on("Payment Tools", "received_or_paid", function(frm) {
	erpnext.payment_tool.check_mandatory_to_set_button(frm);
});

// Set party account name
frappe.ui.form.on("Payment Tools", "customer", function(frm) {
	erpnext.payment_tool.set_party_account(frm);
	erpnext.payment_tool.check_mandatory_to_set_button(frm);
});

frappe.ui.form.on("Payment Tools", "supplier", function(frm) {
	erpnext.payment_tool.set_party_account(frm);
	erpnext.payment_tool.check_mandatory_to_set_button(frm);
});
...
frappe.ui.form.on("Payment Tools", "get_outstanding_vouchers", function(frm) {
		...
		callback: function(r, rt) {
			if(r.message) {
				...
				$.each(r.message, function(i, d) {
					var invoice_detail = frappe.model.add_child(frm.doc, "Payment Tools Detail", "payment_tool_details");
...
frappe.ui.form.on("Payment Tools Detail", "against_voucher_type", function(frm) {
...
// validate against_voucher_type
frappe.ui.form.on("Payment Tools Detail", "against_voucher_no", function(frm, cdt, cdn) {
...
frappe.ui.form.on("Payment Tools Detail", "payment_amount", function(frm) {
	erpnext.payment_tool.set_total_payment_amount(frm);
});

frappe.ui.form.on("Payment Tools Detail", "payment_tool_details_remove", function(frm) {
	erpnext.payment_tool.set_total_payment_amount(frm);
});
...
frappe.ui.form.on("Payment Tools", "make_journal_voucher", function(frm) {
	erpnext.payment_tool.check_mandatory_to_fetch(frm.doc);
